...
export default function CourseDetailClient({ course }: Props) {
  ...
  useEffect(() => {
    const checkAndGenerate = async () => {
      if (!activeChapter.content || !activeChapter.videoId) {
        setLoading(true);
        try {
          const res = await fetch(`/api/courses/${course._id}/chapters/${activeChapterIndex}`, {
            method: "POST",
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden mt-16">
      {/* SIDEBAR */}
      <aside className="w-80 bg-cardbgclr border-r border-borderclr flex-col overflow-y-auto hidden md:flex">
        <div className="p-6 border-b border-borderclr">
          ...
          <p className="text-xs text-graytext mt-1">{chapters.length} Chapters</p>
        </div>
          ...
          {chapters.map((chapter, index) => (
            <button
              ...
              className={`w-full text-left p-4 border-b border-borderclr/50 hover:bg-uibgclr transition-colors flex items-start gap-3
                ${activeChapterIndex === index ? "bg-primary/10 border-l-4 border-l-primary" : ""}`}
            >
              <span className="mt-1 text-primary">
                {activeChapterIndex === index ? <IoPlayCircle size={20} /> : <span className="text-xs font-mono ml-1">{index + 1}</span>}
              </span>
              <div>
                <h3 className={`text-sm font-medium ${activeChapterIndex === index ? "text-primary" : "text-gray-300"}`}>
                  {chapter.chapterName}
                </h3>
        ...
        <header className="h-16 border-b border-borderclr flex items-center justify-between px-6 bg-cardbgclr/50 backdrop-blur">
          ...
          <div className="flex gap-2">
            <Button 
              ...
              className={`text-sm py-1 px-3 ${tab === "video" ? "bg-primary text-black" : "bg-transparent border border-graytext text-graytext"}`}
            >
              Video
            </Button>
            <Button 
              onClick={() => setTab("reading")}
              className={`text-sm py-1 px-3 ${tab === "reading" ? "bg-primary text-black" : "bg-transparent border border-graytext text-graytext"}`}
            >
              Reading
            </Button>
          </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full space-y-6">
              ...
              <div className="text-center space-y-2">
                <h3 className="text-xl font-medium text-white">Creating Chapter Content</h3>
                <p className="text-graytext text-sm">AI is writing notes and finding videos...</p>
              </div>
            </div>
          ) : (
            <>
              {tab === "video" && (
                <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  <div className="bg-cardbgclr p-6 rounded-xl border border-borderclr">
                    ...
                    <p className="text-graytext leading-relaxed">{activeChapter.about}</p>
                  </div>
                </div>
              )}
              ...
              {tab === "reading" && (
                <div className="max-w-4xl mx-auto bg-cardbgclr p-8 rounded-xl border border-borderclr shadow-lg">
                  <article className="prose prose-sm md:prose-base lg:prose-lg prose-invert max-w-none prose-headings:text-primary prose-a:text-blue-400 prose-strong:text-white">
                    <ReactMarkdown>
                      {activeChapter.content || "Content generation failed. Please try again."}
                    </ReactMarkdown>
                  </article>
                </div>
              )}
