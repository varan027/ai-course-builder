    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="space-y-4">
        <div className="flex items-center gap-2 text-primary font-medium text-sm tracking-wide uppercase">
          <span className="bg-primary/10 px-2 py-0.5 rounded">Chapter {index + 1}</span>
          <Separator orientation="vertical" className="h-4 bg-white/10" />
          <span className="flex items-center gap-1.5 text-muted-foreground">
            <Clock className="w-4 h-4" /> {chapter.durationMinutes} mins
          </span>
        </div>
        <h1 className="text-4xl font-bold text-white tracking-tight">{chapter.title}</h1>
      </header>
      ...
      <div className="relative aspect-video w-full overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl">
        {chapter.youtubeVideoId ? (
          ...
          <div className="flex flex-col items-center justify-center h-full space-y-4 bg-zinc-900">
             ...
             <p className="text-muted-foreground text-center px-4">Video content currently unavailable for this module.</p>
             <Button variant="outline" asChild>
                <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(chapter.youtubeQuery)}`} target="_blank">
                  Search Manually
                </a>
             </Button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1 bg-white/5 rounded-2xl p-6 border border-white/10 sticky top-8">
          <form action={async () => { "use server"; await toggleProgress(courseId, index); }}>
