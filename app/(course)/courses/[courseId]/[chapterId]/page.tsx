...
function renderInlineMarkdown(text: string) {
  return text.split(/(`[^`]+`)/g).map((part, index) =>
    part.startsWith("`") && part.endsWith("`") ? <code key={index} className="rounded-md border border-white/[0.08] bg-black/20 px-1.5 py-0.5 font-mono text-[0.9em] text-foreground">{part.slice(1, -1)}</code> : <span key={index}>{part}</span>,
  );
}

function LessonContent({ content }: { content: string }) {
  const blocks = renderLessonContent(content);
  return <div className="max-w-3xl space-y-6">{blocks.map((block, index) => {
...
export default async function SkillPage({ params }: { params: Promise<{ courseId: string; chapterId: string }> }) {
  ...
  const masteryProof = parseMasteryProof({ task: goalSkill.masteryProofTask, proofType: goalSkill.masteryProofType, capabilities: goalSkill.masteryProofCapabilities, evaluationCriteria: goalSkill.masteryProofCriteria });
  const projectState = getProjectProofState(currentStatus, Boolean(skillProgress?.projectStartedAt));

  return <article className="animate-in fade-in duration-500">
    <header className="border-b border-white/[0.07] pb-10 sm:pb-12">
      <div className="flex items-center justify-between gap-4"><p className="text-[10px] font-medium uppercase tracking-[0.22em] text-muted-foreground">{String(index + 1).padStart(2, "0")} / {String(goal.goalSkills.length).padStart(2, "0")}</p>{isCompleted && <span className="flex items-center gap-1.5 text-xs text-primary"><CheckCircle2 className="size-4" /> Mastered</span>}</div>
    ...
    <div className="space-y-5 py-9 sm:py-10">
      <section className="rounded-2xl border border-white/[0.07] bg-white/[0.018] p-6 sm:p-8"><div className="flex items-center gap-2"><BookOpen className="size-4 text-primary" /><p className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground">Learn</p></div><p className="mt-5 max-w-3xl text-base leading-7 text-foreground/90 sm:text-lg">{lessonOverview}</p>{keyIdeas.length > 0 && <div className="mt-8 border-t border-white/[0.07] pt-7"><div className="flex items-center gap-2"><ListChecks className="size-4 text-primary" /><p className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground">Key ideas</p></div><ul className="mt-5 grid gap-3 sm:grid-cols-2">{keyIdeas.map((idea) => <li key={idea} className="rounded-xl border border-white/[0.06] bg-black/10 p-4 text-sm leading-6 text-foreground/90">{idea}</li>)}</ul></div>}</section>
